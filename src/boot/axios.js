...
export default boot(({ app, router, store }) => {
    ...
    api.interceptors.response.use((config) => {
        ...
        } else if (error.response.status == 500 && error.response.data.message == "Masa Pakai Habis atau Melebihi syarat paket"){
            // console.log(error.response)
            store.dispatch("Auth/logout").then(res => {
                Notify.create('Masa Pakai Habis atau Melebihi syarat paket');
            });
        } else if (error.response.status == 500 && error.response.data.message == "Nomor WA Tidak Ditemukan"){
