  ...
  <q-layout>
    ...
    <q-header class="q-pb-xl">
      <q-toolbar class="shadow-1 fixed-top" style="background-color: #1c309b">
        <q-btn @click="$router.back()" no-caps flat style="color: white" round>
    ...
    <q-page-container>
      <q-page class="q-pa-md">
        <!-- Container -->
        <div v-for="(comment, c) in comments" :key="comment.id" class="q-pb-xl">
          <comment-component
...
import CommentComponent from "src/components/course/CommentComponent.vue";
export default {
  props: ["contentid"],
  components: {
  ...
  methods: {
    ...
    deleteComment(payload) {
      let index = this.comments.findIndex((item) => item.id == payload);
