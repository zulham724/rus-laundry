...
export default {
  ...
  methods: {
    ...
    setTextMessage() {
      if (this.order) {
        let url = `${this.APP_URL}/marketplace-detail-owner/${this.product.id}`;
        let tmp = `
          Order via Whatsapp
    ...
    getAnotherProducts(id) {
      this.$store
        .dispatch("MasterProduct/getAnotherProducts", id)
        .then((res) => {
    ...
    async detailProduct(id) {
      ...
      await this.$router.push(`/marketplace-detail-owner/${id}`);
      this.$nextTick().then(() => {
